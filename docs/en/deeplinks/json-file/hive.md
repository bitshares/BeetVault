...
This page shows how to generate JSON files for **Hive**. The envelope is universal — only the transaction construction differs per chain family.
...
async function buildHiveJsonFile(operations) {
...
```jsx
const file = await buildHiveJsonFile(operations);
...
- [Raw Deeplink — Hive](../raw/hive.md) — URL-based alternative
